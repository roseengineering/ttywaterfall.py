...
def parse_args():
    ...
    parser.add_argument('--output', help='output 8-bit wav file')
    parser.add_argument('input', nargs=1, help='input 16-bit wav file')
...
def convert(args):
    ...
    with wave.open(filename, "rb") as wav_in:
        ...
        if sample_width != 2:
            print('16-bit only supported', file=sys.stderr)
            return
    ...
    with wave.open(args.output, "wb") as wav_out:
        ...
        with wave.open(filename, "rb") as wav_in:
            while buf := wav_in.readframes(num_frames):
                ...
                samples = np.round(samples / sample_peak * 127.5) + 128
                samples = np.clip(samples, 0, 255).astype('B')
